...
#check avialable years to see for which countries we have at least 25 years
#HMDL <- HMDL[Year >= 1900,]
#X    <- HMDL[,max(Year)- min(Year)+1, by = list(PopName)]
#X[order(V1),]
#HMDL <- HMDL[!(PopName %in% c('KOR', 'HRV', 'CHL')), ]
...
require(gridExtra)
pdf(file="Figures/FigureA2.pdf",width=13,height=15,pointsize=4)
grid.arrange(KPSS.eta, ADF.eta, ncol=1)
dev.off()