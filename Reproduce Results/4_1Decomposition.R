...
#check consistency
#names(decomp) <- colnames(mat)[-1]
#names(decomp) <- NULL
#vec.decomp1 <-  c(decomp$`1901`,decomp$`1902`,decomp$`1903`)
#vec.decomp2 <-  y[1:(111*3)]
#vec.decomp3 <-  vec.decomp2 - vec.decomp1
...
#Time difference of 6.606153 hours
#check results:

source('4_2Finding_Threshold_ages.R')

#check results
#total.change.decomp <- unlist(lapply(decomp,sum))
#total.change.LT     <-diff(DT[Age == 0,]$ex)
#total.change.decomp - total.change.LT
