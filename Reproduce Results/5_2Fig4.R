...
Fig.4 <- ggplot()+
  ...
  geom_point(data = Scenarios.data,  aes(x=ex.new.optimal.wW,y = h.new.optimal.wW),show.legend = F,shape = 17, col='pink' ,size= 3,alpha=I(1/2))+
  ...
  annotate("text", x = fig.label.x , y =  Scenarios.data[which.max(Scenarios.data$h.new.optimal.wW),]$h.new.optimal.wW, label = "Optimal", col = 'pink', hjust = 0)+
  annotate("text", x = fig.label.x , y =  Scenarios.data[which.max(Scenarios.data$h.new.optimal.wW),]$h.new.optimal.wW-.08, label = "equality", col = 'pink', hjust = 0)+
...
grid.arrange(Fig.4, layout_matrix = lay.Fig1)
dev.off()
