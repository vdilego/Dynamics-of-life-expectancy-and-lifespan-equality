...
#Weights.2010[,dif.age0 := abs(wW_h[1] - wW_h)]
#Weights.2010[,dif.age0 := abs(weights_eo[1] - weights_eo)]
Weights.2010 <- Weights.2010[order(dif.age0),]


# colors for life expectancy
display.brewer.all()
...
Fig2.A <- ggplot(data = Weights.data[ Age<105 & Year %in% c(1751, 1900, 1950, 2010),])+
  ...
  Fig2.B <- ggplot(data = Weights.data[ Age>=5 & Age<105 & Year %in% c(1751, 1900, 1950, 2010),])+
...
Fig2.C <- ggplot(data = Weights.data[ Age<105 & Year %in% c(1751, 1900, 1950, 2010),])+
...
Fig2.D <- ggplot(data = Weights.data[Age>=5 & Age<105 & Year %in% c(1751, 1900, 1950, 2010),])+
...
lay.Fig2 <- rbind(c(1,2),
                  c(3,4))

pdf(file="Figures/Figure2.pdf",width=12,height=12,pointsize=4)
grid.arrange(Fig2.A,Fig2.B,Fig2.C,Fig2.D, layout_matrix = lay.Fig2)
dev.off()



################same figure for other indicators for supplementary material
